...
class MergeVQModel(L.LightningModule):
    def __init__(self,
                ddconfig,
                lossconfig,
                k2lconfig = None,
                ...
                ckpt_path = None,
                ignore_keys = [],
                image_key = ["image", "image_aug_w", "image_aug_s"],
        ...
        super().__init__()
        self.image_key = image_key if isinstance(image_key, list) else [image_key]
        ...
        if pretrain_teacher is not None:
            try:
                self.dist_model = AutoModel.from_pretrained(pretrain_teacher)
                for param in self.dist_model.parameters():
                    param.requires_grad = False
            except:
                print(f'Fail to load pretrained model from {pretrain_teacher}.')
                assert self.dist_model != None
        self.cls_loss_type = cls_loss_type
        assert cls_loss_type in ['kl', 'ce', 'mse']
    ...
    def encode(self, x):
        """encoding and quatization"""
        h = self.encoder(x)
        (quant, emb_loss, info), loss_breakdown = self.quantize(h, return_loss_breakdown=True)

        return quant, emb_loss, info, loss_breakdown

    def encode_tome(self, x):
        """ToMe encoding and quatization with unmerge"""
        h = self.encoder(x)
       
        (quant, emb_loss, info), loss_breakdown = self.quantize(h, return_loss_breakdown=True)
        if self.encoder.attn._tome_info['class_token'] and self.encoder.attn._tome_info['source'] is not None:
            source = self.encoder.attn._tome_info['source'][:, 1:, 1:]
            quant = torch.flatten(quant, start_dim=2)
            quant = quant.permute(0, 2, 1)
            quant = token_unmerge(quant, source).permute(0, 2, 1)
            B, D, L = quant.shape
            quant = quant.reshape(B, D, int(L**0.5), -1)
        
        return quant, emb_loss, info, loss_breakdown

    def decode(self, quant):
        """unmerging and decoding"""
    ...
    def distill_cls_loss(self, tea_cls_token, stu_cls_token, temperature=4):
        """distillation kl loss with teacher model"""
        prob_tea = F.softmax(tea_cls_token / temperature, dim=-1)
        prob_stu = F.log_softmax(stu_cls_token / temperature, dim=-1)
        loss = F.kl_div(prob_stu, prob_tea, reduction='batchmean') * (temperature ** 2)
        return loss.mean()

    def distill_mse_loss(self, tea_cls_token, stu_cls_token):
        """distillation mse loss with teacher model"""
        norm_tea = torch.nn.functional.normalize(tea_cls_token, dim=-1)
        norm_stu = torch.nn.functional.normalize(stu_cls_token, dim=-1)
        loss = -(norm_stu * norm_tea)
        return loss.mean()

    def supervised_cls_loss(self, cls_label, stu_cls_token, smoothing=0.1):
        """label smoothing cross-entropy with labels"""
        prob_stu = F.log_softmax(stu_cls_token, dim=-1)
        nll_loss = -prob_stu.gather(dim=-1, index=cls_label.unsqueeze(1))
        nll_loss = nll_loss.squeeze(1)
        smooth_loss = -prob_stu.mean(dim=-1)
        loss = (1. - smoothing) * nll_loss + smoothing * smooth_loss
        return loss.mean()

    def get_cls_loss(self, x_aug, label=None):
        if self.dist_model is not None:  # distillation loss
            tea_cls_token = self.dist_model(x_aug[0]).pooler_output
            stu_cls_token = self.encoder(x_aug[-1], return_head=True)
            if self.cls_loss_type == 'kl':
                cls_loss = self.distill_cls_loss(tea_cls_token, stu_cls_token)
            elif self.cls_loss_type == 'mse':
                cls_loss = self.distill_mse_loss(tea_cls_token, stu_cls_token)
        else:  # supervised cls loss
            stu_cls_token = self.encoder(x_aug[-1], return_head=True)
            cls_loss = self.supervised_cls_loss(label, stu_cls_token)
        return cls_loss, stu_cls_token

    def forward(self, x, x_aug=None, label=None):
        quant, diff, _, loss_break = self.encode(x)
        #print(quant.shape)
        dec = self.decode(quant)
        if x_aug is None and label is None:
            return dec, diff, loss_break
        else:
            cls_loss, stu_cls_token = self.get_cls_loss(x_aug, label)
            return dec, diff, loss_break, cls_loss

    def get_input(self, batch, keys=None):
        """split input images and class_label"""
        imgs = list()
        for k in keys:
            x = batch[k]
            if len(x.shape) == 3:
    ...
    def training_step(self, batch, batch_idx):
        x, label = self.get_input(batch, self.image_key)
        x, x_aug = x[0], x[1:]
        self.update_encoder_merge_ratio(batch_idx)
        xrec, eloss, loss_break, cls_loss = self.forward(x, x_aug, label)
        ...
        # optimize generator
        aeloss, log_dict_ae = self.loss(eloss, loss_break, x, xrec, 0, self.global_step, cls_loss=cls_loss,
        ...
        discloss, log_dict_disc = self.loss(eloss, loss_break, x, xrec, 1, self.global_step, cls_loss=cls_loss,
                                            last_layer=self.get_last_layer(), split="train")
        if self.accumulate_grad_batches > 1:
    ...
    def _validation_step(self, batch, batch_idx, suffix=""):
        ...
        cls_loss, _ = self.get_cls_loss(x_aug, label)
        aeloss, log_dict_ae = self.loss(eloss, loss_break, x, x_rec, 0, self.global_step, cls_loss,
                                        last_layer=self.get_last_layer(), split="val"+ suffix)

        discloss, log_dict_disc = self.loss(eloss, loss_break, x, x_rec, 1, self.global_step, cls_loss,
                                            last_layer=self.get_last_layer(), split="val" + suffix)
    ...
    def log_images(self, batch, **kwargs):
        log = dict()
        x = self.get_input(batch, self.image_key)[0]  # only eval 'image'
        x = x.to(self.device)
        xrec, _ = self.forward(x)
        if x.shape[1] > 3:
