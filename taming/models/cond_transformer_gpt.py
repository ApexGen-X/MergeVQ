...
class Net2NetTransformer(L.LightningModule):
    ...
    def state_dict(self, *kwargs, destination=None, prefix='', keep_vars=False):
        return {k: v for k, v in super().state_dict(*kwargs, destination, prefix, keep_vars).items() \
    ...
    def encode_to_z(self, x):
        if getattr(self.first_stage_model, "encode_tome", None) is not None:
            quant_z, _, indices, _ = self.first_stage_model.encode_tome(x)
        else:
    ...
    def get_input(self, key, batch):
        x = batch[key]
        if len(x.shape) == 3:
            x = x[..., None].permute(0, 3, 1, 2).contiguous()
        # if len(x.shape) == 4:
    ...
    def shared_step(self, batch, batch_idx):
        ...
        logits, target = self.forward(x, c)
    ...
    def training_step(self, batch, batch_idx):
        ...
        min_tlr, max_tlr, min_twd, max_twd = self.lr_wd_annealing(
            self.learning_rate, self.weight_decay, self.twde, g_it, wp_it, max_it, wp0=self.wp0, wpe=self.wpe)
        ## --------------------------------------------
