...
from taming.data.base import ImagePaths, MultiImagePaths
...
class ImageNetBase(Dataset):
    ...
    def _load(self):
        ...
        img_size = retrieve(self.config, "size", default=256)
        if isinstance(img_size, int):
            self.data = ImagePaths(self.abspaths,
                            labels=labels, size=img_size,
                            is_training=self.is_training)
        else:
            augmentation = retrieve(self.config, "augmentation", default='default')
            self.data = MultiImagePaths(self.abspaths,
                            labels=labels, size=img_size,
                            is_training=self.is_training,
...
class ImageNetTrain(ImageNetBase):
    ...
    expected_length = 1281167

    def _prepare(self):
        self.is_training = retrieve(self.config, "ImageNetTrain/is_training",
                                    default=True)
        self.cachedir = retrieve(self.config, "cachedir", default=os.path.expanduser(".cache/imagenet"))
        self.root = os.path.join(self.cachedir, self.NAME)
        self.datadir = self.root
        if self.config["subset"] is not None: # for training in subset
            self.txt_filelist = os.path.join(".cache/imagenet", "{}_{}.txt".format(self.NAME, self.config["subset"]))
        else:
        ...
        if not bdu.is_prepared(self.root):
            ...
            if not os.path.exists(datadir):
                path = os.path.join(self.cachedir, self.FILES[0])
...
class ImageNetValidation(ImageNetBase):
    ...
    def _prepare(self):
        self.is_training = retrieve(self.config, "ImageNetValidation/is_training",
                                    default=False)
        self.cachedir = retrieve(self.config, "cachedir", default=os.path.expanduser(".cache/imagenet"))
        self.root = os.path.join(self.cachedir, self.NAME)
        self.datadir = self.root
...
def get_preprocessor(size=None, is_training=False, additional_targets=None,
                     crop_size=None):
...
class BaseWithDepth(Dataset):
    ...
    def __init__(self, config=None, size=None, is_training=False,
...
class ImageNetTrainWithDepth(BaseWithDepth):
    # default to is_training=True
    def __init__(self, is_training=True, sub_indices=None, **kwargs):
        self.sub_indices = sub_indices
        super().__init__(is_training=is_training, **kwargs)
...
class RINTrainWithDepth(ImageNetTrainWithDepth):
    def __init__(self, config=None, size=None, is_training=True, crop_size=None):
        sub_indices = "30-32, 33-37, 151-268, 281-285, 80-100, 365-382, 389-397, 118-121, 300-319"
        super().__init__(config=config, size=size, is_training=is_training,
                         sub_indices=sub_indices, crop_size=crop_size)


class RINValidationWithDepth(ImageNetValidationWithDepth):
    def __init__(self, config=None, size=None, is_training=False, crop_size=None):
        sub_indices = "30-32, 33-37, 151-268, 281-285, 80-100, 365-382, 389-397, 118-121, 300-319"
        super().__init__(config=config, size=size, is_training=is_training,
                         sub_indices=sub_indices, crop_size=crop_size)
...
class ImageNetScale(Dataset):
    def __init__(self, size=None, crop_size=None, is_training=False,
