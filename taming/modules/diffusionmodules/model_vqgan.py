...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, in_channels, num_res_blocks, z_channels, ch_mult=(1, 2, 2, 4), 
                resolution, double_z=False, **ignore_kwargs):
        super().__init__()
        ...
        for res_idx in range(self.num_res_blocks[-1]):
    ...
    def forward(self, x):
        ...
        ## mid 
        for res in range(self.num_res_blocks[-1]):
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, in_channels, num_res_blocks, z_channels, ch_mult=(1, 2, 2, 4), 
                resolution, double_z=False, **ignore_kwargs) -> None:
        super().__init__()
...
if __name__ == "__main__":
    ...
    ddconfig = dict(
        ...
        # ch = 64,
        # ch_mult = [1,2,4,8],  # num_down = len(ch_mult)-1
        # num_res_blocks = [3,5,7,3],
        double_z =False,
    )
    encoder = Encoder(**ddconfig)
    decoder = Decoder(**ddconfig)
    
    input = torch.rand(1, 3, resolution, resolution)
    flop = FlopCountAnalysis(encoder, input)
    # print(encoder)
    print(flop_count_table(flop, max_depth=4))
    print('MACs (G) of Encoder: {:.3f}'.format(flop.total() / 1e9))

    curr_res = resolution // 2 ** (len(ddconfig['ch_mult']) - 1)
    input = torch.rand(1, 18, curr_res, curr_res)
    flop = FlopCountAnalysis(decoder, input)
    # print(decoder)
    print(flop_count_table(flop, max_depth=4))
