...
def make_block(channels: int, kernel_size: int, norm_type: str, norm_eps: float, using_spec_norm: bool) -> nn.Module:
    if norm_type == 'bn':
        ...
        norm = nn.SyncBatchNorm(channels, eps=norm_eps, process_group=None)  # fix
    elif norm_type == 'gn':
...
class FrozenDINOSmallNoDrop(nn.Module):
    """
    Frozen DINO ViT without any dropout or droppath layers (eval node only),
...
if __name__ == '__main__':
    ...
    inp = torch.linspace(-2, 2, 2 * 3 * 256 * 256).reshape(2, 3, 256, 256)
